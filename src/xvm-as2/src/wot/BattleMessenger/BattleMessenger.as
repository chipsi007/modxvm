...
{   
    ...
    function _onPopulateUIImpl() {
		...
		Logger.add("[AS2][BattleMessenger/BattleMessenger]_onPopulateUIImpl():END");
        }
    }
    ...
    function _onRecieveChannelMessageImpl(cid:Number, message:String, himself:Boolean, targetIsCurrentPlayer:Boolean)
    {
        ...
        if (Config.config.battleMessenger.enabled && debugMode) {
            var log:Object = { 
                msg: message,
                display: displayMsg
            };
        ...
        if (displayMsg || (Config.config.battleMessenger.enabled && debugMode)) { 
            base._onRecieveChannelMessage(cid, message, himself, targetIsCurrentPlayer);
        }
		Logger.add("[AS2][BattleMessenger/BattleMessenger]_onRecieveChannelMessageImpl():END");
    }
    ...
    /**
     * Check if we should block/filter player by block/filter section in config
     * @param   player
    ...
    private function blocker(player:Player, block:Boolean):Boolean {
        ...
        this.sendDebugMessage("blocker, Block checking mode:"+block+", Decision:"+toBlock, false)
        return toBlock;
    }
    ...
    private function blockerHelper(player:Player, type:String, reason:String, block:Boolean) {
		...
		//check blocking by dead/alive stat
        if (type == "all") { 
			toBlock = true;  
		} else if (type == "alive" && !dead) { 
			toBlock = true; 
		} else if (type == "dead"  && dead) { 
			toBlock = true;  
		} else {
			toBlock = false;
		}

		// check for block/filter decision
        if (toBlock && block) {
    ...
    public function sendDebugMessage(text:String, ignoreDebugMode:Boolean) {
        if (Config.config.battleMessenger.enabled && debugMode && text.length > 0) {
            Logger.add(text);
    ...
    /**
	 * Check player membership in clan
     * @param   player: player object
    ...
    /**
	 * Check player membership in squad
     * @param   player: player object
